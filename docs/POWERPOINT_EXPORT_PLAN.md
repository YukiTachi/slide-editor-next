...
#### 6.14 エクスポート設定モーダル
- [x] `components/PowerPointExporter/PowerPointExporterModal.tsx` を作成
  - [x] 画像品質の選択
  - [x] ページ番号の有無の選択
  - [x] **スライドサイズの選択肢は設けない**（サイズはエディタの設定に追従する方針と矛盾するため）
- [x] 設定を保存してエクスポートに反映
...
**Phase 5 検証結果（2026-08-19、headless Chromium + CDPによる実UI検証）**:
- ✅ 設定モーダル: メニューの「PowerPoint出力」で開き、画像品質3段階とページ番号有無を選んで「出力」で生成。設定はlocalStorage（`slideEditor_powerpointExportSettings`）に保存され次回に引き継がれる。スライドサイズは変更不可の表示のみ（エディタ設定に追従する方針を明示）
- ✅ 画像最適化: 2000×1500pxのPNG（1.25MB）が「低」でJPEG 27KBに縮小され、pptx全体は1.29MB→74KB。**表示サイズは原寸基準のまま不変**（圧縮後の画素数に引きずられない）
- ✅ 透過画像はJPEG化せずPNGを維持（46KB→19KB）。SVGはベクタのまま埋め込み
- ✅ ページ番号OFFでスライド番号プレースホルダが出力されない
- ✅ エラーハンドリング: スライドのないHTMLは「PowerPoint出力に失敗しました。/ スライド（&lt;div class="slide"&gt;）が見つかりません」と具体的な理由を表示。取得できない画像・壊れたグラフJSONは該当要素のみスキップして出力を継続し、スライド番号付きの警告一覧をalertで通知
- ✅ 進捗表示: 「PowerPointを準備中…」→「生成中…（n/N）」→「書き出し中…」→「ダウンロードしました（N枚）」をステータスバーに表示。5スライドごとにUIスレッドへ制御を返す
...
- [x] **棒グラフの棒ごとの色分け**: 1系列に色配列（棒4本に4色等）が指定されたグラフは、系列単位の色変換のため全棒が先頭の色になる。単一系列かつ `backgroundColor` が配列の場合は pptxgenjs の `varyColors` 相当の指定（`chartColors` に配列全体を渡す）で棒ごとの色を再現する
- [x] **インラインのfont-size指定**: 要素の `style="font-size: 28px"` 等が無視され、役割既定のサイズ（本文18pt）で出力される。px→ptに換算して `style.fontSize` に反映する
- [x] **リスト項目内のインライン書式**: `<li>` 内の `.highlight` / `<strong>` 等がプレーンテキスト化される（リストは項目文字列として抽出するため）。リスト項目にもラン変換を適用する場合は、pptxgenjsのbullet付きラン配列（項目ごとに `breakLine` + `bullet`）へ変更が必要
...
- [x] 基本的なテキストスライドが変換される
- [x] 画像を含むスライドが変換される
- [x] 表を含むスライドが変換される
- [x] グラフを含むスライドが変換され、PowerPoint上で編集できる
- [x] 数式・コードブロックを含むスライドが変換される（本文テキストの重複なし）
- [x] 2分割レイアウト（`slide-split` / two-column両方）が正しく変換される
- [x] スライドサイズ設定（A4横 / 16:9）がpptxのページサイズに反映される
- [x] 複数のスライドが正しく変換される
- [x] PowerPointで開いて正しく表示される
- [x] 生成された.pptxのOOXML構造が妥当（開発環境では `unzip` + XML確認、または `python-pptx` での読み込み検証が可能）
- [x] ファイルサイズが適切である
- [x] エラーハンドリングが機能する
...
### 技術的な確認事項
- [x] 既存のコードベースの理解
- [x] `extractSlides()` 関数の動作確認（`lib/slideReorder.ts:6` に実在を確認済み）
- [x] 画像処理ロジックの理解
- [x] 表処理ロジックの理解
- [x] pptxgenjsのドキュメント確認（特に `addChart()` の対応グラフタイプとChart.jsタイプとの対応関係）
- [x] 実装環境の準備
...
## 15. 実装完了状況（2026-08-19時点）

**Phase 1〜5はすべて実装・検証完了。** 本機能は実用可能な状態にある。

| Phase | 内容 | 状態 |
|---|---|---|
| 1 | テキスト（title/subtitle/text/list）・サイズ追従・メニュー統合 | ✅ 完了 |
| 2 | 画像（base64 / ローカルストレージ / 外部URL） | ✅ 完了 |
| 3 | 表（5スタイル・ヘッダー識別・キャプション） | ✅ 完了 |
| 3.5 | グラフ（ネイティブ変換 + 画像フォールバック） | ✅ 完了 |
| 3.6 | 数式（LaTeXソース）・コードブロック | ✅ 完了 |
| 4 | テンプレート色・インライン書式のラン変換 | ✅ 完了 |
| 5 | 設定モーダル・エラーハンドリング・画像最適化・忠実度ギャップ解消 | ✅ 完了 |

**残っている未実装項目**（いずれも需要が出たときに対応する発展項目）:
- KaTeX数式のオフスクリーン描画による画像化（現状はLaTeXソースをテキストで挿入）
- シンタックスハイライトの配色再現（7.6の制約どおり非対応）
- 10章の今後の拡張案（アニメーション・PowerPointテンプレート適用・スピーカーノート）
