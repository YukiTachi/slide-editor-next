...
## 10. 検証結果（2026-08-16 実施）

Phase 1・Phase 2 の検証を完了。A4横向き・16:9の両方で、テキスト（ベクター・選択可）・
蛍光ペン等の背景装飾・表（縞/罫線/ヘッダー背景）・グラフ（Chart.js 2種）・数式（KaTeX
ブロック/インライン/キャプション）・コードブロック（ダーク背景/ハイライト/行番号）・
分割レイアウト+画像が、1スライド=1ページ・余白0・末尾空白ページなしで出力されることを
実機PDFで確認。カスタムCSSテンプレート適用状態でも動作確認済み。

実装中に発見・修正した問題:
- コンテンツ中のコメントに「/body閉じタグ」の文字列があると、スクリプト注入が
  コメント内に吸われて全リッチ要素が空になる → `insertBeforeClosingBody`（lastIndexOf方式）で修正
- 表・グラフ・コード・数式の基本スタイルが `public/css/slide-styles.css` にしかなく
  プレビュー/PDFの両方で欠落 → `lib/slideComponentStyles.ts` に切り出して注入
- スライド高さを超えるコンテンツはflex縮小によりスクロールコンテナがクリップされ、
  スクロールバーがPDFに写る → スクロールバーのみ印刷時非表示化（クリップ挙動はプレビューと同一のため仕様）

既知の残課題（PDF固有ではなくプレビューと同一の表示）:
- Prismのデフォルトテーマとtomorrowテーマの二重読込により、演算子トークンに
  半透明の背景ボックスが付く（`htmlProcessor` のテーマ読込を1つに絞れば解消）
- スライド高さを超えたコンテンツは切れる（固定サイズスライドの仕様。コンテンツ側で調整）
- `htmlProcessor.loadActualCSS()` は未使用のデッドコード（将来整理）
